...
pub fn use_items(ecs: &mut SubWorld, commands: &mut CommandBuffer, #[resource] map: &mut Map) {
    ...
    <(Entity, &ActivatedItem)>::query()
        ...
        .for_each(|(entity, activate)| {
            ...
            commands.remove(activate.item); // delete the item so it can't be used again
            commands.remove(*entity); // delete the activation command so it doesn't fire again
        });
